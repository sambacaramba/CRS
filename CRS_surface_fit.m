...
%% used functions CRS_SurFit, CRS_FineTuneSurf 
res_loc=1;
...
%% use a smoothed surface (median filter) for the test to remove "surface spikes" from the fit
    ...
    if surfval < 0 
        if option == 1
    [fitsurf number]= CRS_FineTuneSurf(fitsurf,b_surf,fn_b,savefold);
    else 
        end 
    end

%gather stats from the fit (needed if CRS_Fine_TuneSurf was used)
    difsurf =(fitsurf-(-b_surf));
